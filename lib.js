...
function wrapText(context, text, x, y, maxWidth, lineHeight,col) {
	...
	for(var n = 0; n < words.length; n++) {
	  ...
	  if(testWidth < maxWidth && counter < 1){
		maximumWidth = maxWidth/2;
	  }else{
		  maximumWidth = maxWidth;
	  }
	  if (testWidth >= maximumWidth && n > 0) {
		showText(line,x,y,20,col);
		line = words[n] + ' ';
		y += lineHeight;
		counter++;
	  }
...
function showText(text, X, Y, Size, colour = "rgb(0, 0, 0)", bold = false, stroke = false){
	...
	if(stroke === false){
		c.fillStyle=colour;
		c.fillText(text, (X-Camera.x*scale)*Camera.scale, (Y-Camera.y*scale)*Camera.scale);
	}
	if(stroke === true){
		c.lineWidth = Size/25;
		c.strokeStyle = colour;
		c.strokeText(text, (X-Camera.x*scale)*Camera.scale, (Y-Camera.y*scale)*Camera.scale)
	}
}
...
class image{
	...
	drawRotatedImg(X, Y, W, H, alpha, rotation, rotateAroundX = 0, rotateAroundY = 0){
		c.save();
		c.translate((X*scale-Camera.x)*Camera.scale, (Y*scale-Camera.y)*Camera.scale);
		c.rotate(rotation);
		this.drawImg(-rotateAroundX, -rotateAroundY, W*scale*Camera.scale, H*scale*Camera.scale, alpha);
		c.restore();
	}
}
class spriteSheet{
    ...
    draw(alpha = 1){
		...
		c.globalAlpha = alpha;
        c.drawImage(this.img,this.sheetX,this.states[this.state][0],this.w,this.h,(this.x*scale-Camera.x)*Camera.scale,(this.y*scale-Camera.y)*Camera.scale,this.draww*scale*Camera.scale,this.drawh*scale*Camera.scale);
        c.restore();
    }
...
function drawLine(x1,y1,x2,y2,col){
    c.beginPath();
    c.strokeStyle = col;
    c.moveTo(x1*scale-Camera.x*Camera.scale,y1*scale-Camera.y*Camera.scale);
...
function drawRect(x,y,w,h,col,fill,fillcolor,alpha,outline=true){
    ...
    c.globalAlpha = alpha;
    c.beginPath();
    c.rect((x*scale-Camera.x)*Camera.scale,(y*scale-Camera.y)*Camera.scale,w*scale*Camera.scale,h*scale*Camera.scale);
    if (fill){
...
function drawCircle(x,y,r,col,fill,fillcolor,alpha){
    ...
    c.globalAlpha = alpha;
    c.beginPath();
    c.arc((x*scale-Camera.x)*Camera.scale,(y*scale-Camera.y)*Camera.scale,r*scale*Camera.scale,0,360,false);
    if (fill){
        c.fillStyle = fillcolor;
        c.fill();
    }
